...
logger = logging.getLogger("openwiki_server_sdk")
...
def handle_response(response: httpx.Response, *, trace_id: str, raise_for_error: bool) -> Envelope:
    ...
    if response.status_code >= 400:
        ...
        except OpenWikiServerProtocolError:
            raise OpenWikiServerHTTPStatusError(
    ...
    try:
        envelope = parse_envelope(text)
    except OpenWikiServerProtocolError as exc:
        raise OpenWikiServerHTTPStatusError(
        ...
        for attempt in range(self._max_retries + 1):
            try:
                ...
                if not can_retry(method, body) or attempt >= self._max_retries:
                    raise OpenWikiServerTimeoutError(f"请求超时: {method} {url}", trace_id=trace_id) from exc
                sleep_backoff(attempt)
                continue
            except httpx.ConnectError as exc:
                if not can_retry(method, body) or attempt >= self._max_retries:
                    raise OpenWikiServerConnectionError(f"连接失败: {method} {url}", trace_id=trace_id) from exc
                sleep_backoff(attempt)
                continue
            except httpx.HTTPError as exc:
                raise OpenWikiServerConnectionError(f"传输失败: {method} {url}: {exc}", trace_id=trace_id) from exc
            if response.status_code in _RETRYABLE_STATUS_CODES and attempt < self._max_retries and can_retry(method, body):
                sleep_backoff(attempt)
                continue
            return response
        raise OpenWikiServerConnectionError(f"重试耗尽: {method} {url}", trace_id=trace_id)
